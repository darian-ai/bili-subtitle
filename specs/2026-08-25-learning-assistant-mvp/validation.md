...
| 4 | Chrome；长视频；多 P；AI 单轨 | 未完成 | P10-002（重开）、P10-005 | P1 检查期间切换 P2 后迟到结果污染；任务无法停止；因阻断未测试服务重启 |
| 5 | Edge、服务重启、人工/多轨及三标签页按剩余覆盖组合执行 | 待执行 | 待记录 | 待执行；覆盖不足时增加第 6 次 |
...
- Transcript 准备请求 schema 不含 CID/标题；测试证明后端重新解析 BV/P→CID。提交错误 revision/expected BV/P 返回稳定 `transcript_source_mismatch`，Provider 调用数为零。
- workspace 同时覆盖“只有 Transcript、没有指南”和“已有指南”页面；已有指南始终显示其绑定 revision，新检查 revision 只显示为待生成版本。
