...
- 审计发现并修复三项完成阻断：平台明确广告但正文地址为空的轨道现在保留身份并在下载阶段归类为“不可访问”，不再与合法空轨道集合混为“无字幕”；每页未消费的签名地址在结束、无语言匹配和单轨道流程退出时显式清除；`path_history` 按安全相对文件对保留多轨历史，只在稳定轨道 ID 精确匹配或当前与历史均为单一语言候选时复用，任何同语言多轨歧义均不猜测、不覆盖。
- 历史 manifest 只接收白名单、类型正确且文件对安全的字段；父目录/绝对路径不能参与复用。跨单页命令导致当前 `pages` 快照变化时，历史仍可按 CID 恢复唯一映射；同语言多轨轨道 ID 与展示字段同时波动时产生确定的新身份文件并保留全部旧结果。
- 正文地址来自一次播放器发现响应，每条选定轨道最多消费一次；未选地址在页末清除。固定响应直接覆盖混合“不可访问/有效”轨道继续处理、全不可访问整体失败、重复身份拒绝及清理后的地址不可再次消费。地址不进入领域模型、异常、日志、文件名或 manifest。
...
1. **单命令且无需手工 Cookie：通过。** 自动登录恢复测试与真实 Windows 扫码、凭据复用记录直接证明。
2. **AI 每条所选轨道生成 JSON/SRT：通过。** 固定 AI HTTP—文件闭环及真实 AI 投稿生成和 manifest 核对直接证明。
3. **单/多分集、多语言、同语言多轨不覆盖：通过。** 流程矩阵、歧义历史回归及真实 92 页/单页验收直接证明。
4. **SRT 忠实对应原始 JSON：通过。** 顺序、时间、空格、换行测试及真实产物从原始片段逐字节重建直接证明。
5. **无字幕与访问失败区分：通过。** 合法空集合、空正文地址、HTTP/平台拒绝和结构错误分类测试直接证明。
6. **重复、部分失败与退出码：通过。** 四种文件存在组合、故障隔离及实际控制台 `0/1/2` 测试直接证明。
7. **测试和实际输出无秘密：通过。** 统一金丝雀、归档扫描、真实产物脱敏扫描及人工终端检查直接证明。
8. **无媒体下载、ASR、OCR 或绕过：通过。** 源码、依赖、脚本、CI、文档和归档调用路径审计直接证明。

阶段六 requirements 的全部范围与决策、八组 plan、Roadmap 阶段六验收、Tech Stack 架构/依赖/离线/安全/交付约束及 V1 完成定义均已通过。阶段六可合并；该结论只覆盖 Constitution 规定的本地 wheel/sdist 与 `uv tool install`，不表示已发布包索引或 GitHub Release。
...
### 2026-08-21 真实 AI 字幕兼容性修复与验收

- 首次使用真实平台正常权限验收时，认证与元数据成功，但 P01 字幕轨道发现失败并返回 `2`；未生成轨道文件。脱敏结构诊断确认播放器响应已不再保证旧 `is_ai` 字段，而以取值同为 `0/1` 的 `type` 字段表示人工/AI 轨道；其余必需字段类型保持不变。
- 字幕适配器现在优先兼容旧 `is_ai`，字段缺席时读取当前 `type`，两种格式都严格限定为布尔值或整数 `0/1`；未放宽轨道身份、下载地址、权限或可信主机约束。固定响应回归测试覆盖当前字段形态且不含真实投稿、账号、正文或签名地址。
- 修复后在仓库外专用临时目录通过公开 CLI 重试同一真实 AI 字幕分集：退出码 `0`，发现 1 个 AI 轨道，生成 1 份原始 JSON、1 份 SRT 和 1 份 manifest，均成功发布。
- 程序化忠实性检查解析原始 JSON 的 186 个片段，确认 186 个文本均非空、时间非负且结束不早于开始；用正式 SRT 渲染规则从同一片段序列重建结果，与落盘 SRT 逐字节等价，并确认 manifest 将轨道标记为 `ai` 和成功。检查未输出或记录标题、投稿标识、账号、字幕正文、Cookie、二维码或签名地址；专用临时产物检查后已删除。
- 修复与上述脱敏验收记录所在提交 `49c8da3dffd71e19c64522dae9ba3e4886d34e9e` 的 Windows Quality run [`32445681348`](https://github.com/darian-ai/bili-subtitle/actions/runs/32445681348) 成功；测试、Ruff、格式、strict Pyright、构建、归档/sdist 重建校验及仓库外隔离安装均通过。
...
### 2026-08-21 真实多分集兼容性修复与回归

- 首次真实 `--all-pages` 验收处理 92 个分集，出现大量轨道发现/正文失败，且公开入口错误返回 `0`。脱敏诊断确认平台会返回元数据合法但正文地址暂时为空的轨道；可用地址仍全部属于既有可信域，下载响应为 HTTP 200 且正文片段列表结构有效。修复未放宽权限、协议、端口或可信主机。
- 字幕适配器将空正文地址分类为当前不可用；同页其他有效轨道仍处理。有效签名地址只在适配器私有内存中短暂保存并单次消费，避免为下载再次请求播放器接口导致地址集合变化；地址不进入领域模型、日志、异常、manifest 或文件名。
- 脱敏重复采样确认平台轨道数值 ID 会轮换，正文可见性、显示元数据及语言代码也可能跨请求波动。输出流程因此以经过安全校验的 manifest `path_history` 保留历史 CID/语言到相对文件名的唯一映射；唯一匹配时复用首次路径，同语言多轨道等含糊情况不猜测。当前 `pages` 仍只记录本次快照，不把暂不可用轨道伪报为成功。
- 公开入口改为非 standalone Typer 调用并显式返回其整数结果；控制台脚本和 `python -m` 均保留真实 `0|1|2`。fresh Python 子进程固定测试证明部分成功状态不再被入口吞掉。
- 固定响应新增空地址与同页有效轨道并存、全空正常无字幕、轮换 ID 路径复用、暂不可用后恢复、恶意父目录路径拒绝及 fresh 子进程退出码覆盖。全量 200 项测试通过，覆盖率 91.15%；Ruff lint/format、strict Pyright 和 `git diff --check` 均返回 `0`。
- 仓库外专用临时目录的真实全分集连续三轮均处理 92 个分集，分集失败与轨道失败均为 0，无 traceback，退出码均为 `0`。首轮成功写入；后续轮次对历史可见轨道跳过或覆盖，对此前暂不可见而新出现的轨道新增文件，manifest/产物计数一致且秘密扫描通过。
- 独立 `--page` 在连续可用窗口中依次验证首次写入、默认跳过和 `--force` 替换，三次退出码均为 `0`。平台暂不可用窗口被稳定分类为无字幕并正常返回 `0`，不会误报结构失败。
- `uv build`、归档/秘密/sdist 独立重建校验及从 wheel 的仓库外隔离工具安装均返回 `0`。记录提交 `bc7bc6c1b1316c7294b201e7f7af611b82557556` 的 Windows Quality run [`32446958766`](https://github.com/darian-ai/bili-subtitle/actions/runs/32446958766) 成功；锁定环境、200 项测试、Ruff、格式、strict Pyright、构建、归档/sdist 校验和仓库外隔离安装全部通过。阶段仍等待最终独立完成审计，不在本记录中声明完成。
